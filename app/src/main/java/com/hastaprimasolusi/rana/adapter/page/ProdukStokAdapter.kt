...
import kotlinx.android.synthetic.main.item_produk_stokdetail.view.*
import kotlin.properties.Delegates

/**
 * Created By maasrahman on 5/6/20
 */
class ProdukStokAdapter(private val listener:(ProdukModel?) -> Unit)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return ProdukStokHolder(LayoutInflater.from(parent.context).inflate(R.layout.item_produk_stokdetail, parent, false))
    }
    ...
    inner class ProdukStokHolder(itemView: View) : RecyclerView.ViewHolder(itemView){
        fun bindItem(model: ProdukModel?, listener: (ProdukModel?) -> Unit){
            itemView.txtKodeProd.text = model?.pRODCODE
            itemView.txtNama.text = model?.pRODNAME
            itemView.txtKategori.text = model?.pRODCATEGORYNAME
            itemView.imgProduk.loadImage(model?.pRODPICMEDIUM.toString())
            var adapter = model?.pRODPRICE?.let { ProductPriceAdapter(it) }
            itemView.recyclerPrice.layoutManager = LinearLayoutManager(itemView.context)
            itemView.recyclerPrice.itemAnimator = DefaultItemAnimator()
            itemView.recyclerPrice.adapter = adapter
//            itemView.txtStok.visibility = View.GONE

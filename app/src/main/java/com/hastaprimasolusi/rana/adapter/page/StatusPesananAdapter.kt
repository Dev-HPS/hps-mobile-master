 ...
 */
class StatusPesananAdapter: RecyclerView.Adapter<StatusPesananAdapter.StatusPesananHolder>() {

    private var listData: List<HistoryDetailStatusModel> by Delegates.observable(emptyList()) { _, _, _ ->
        notifyDataSetChanged()
    }

    fun updateData(status: List<HistoryDetailStatusModel>) {
        listData = status
    }
    ...
    inner class StatusPesananHolder(itemView: View) : RecyclerView.ViewHolder(itemView){
        fun bindItem(model: HistoryDetailStatusModel, pos: Int){
            if(pos == 0){

...
import com.hastaprimasolusi.rana.data.network.response.canvas.RiwayatCnvsStatusModel
 ...
 */
class StatusPesananCnvsAdapter: RecyclerView.Adapter<StatusPesananCnvsAdapter.StatusPesananHolder>() {

    private var listData: List<RiwayatCnvsStatusModel> by Delegates.observable(emptyList()) { _, _, _ ->
        notifyDataSetChanged()
    }

    fun updateData(status: List<RiwayatCnvsStatusModel>) {
        listData = status
    }
    ...
    inner class StatusPesananHolder(itemView: View) : RecyclerView.ViewHolder(itemView){
        fun bindItem(model: RiwayatCnvsStatusModel, pos: Int){
            if(pos == 0){
            ...
            itemView.txtTanggal.text = convertDateTime(model.tANGGAL.toString(), "dd MMM yyy HH:mm")
        }
    }
}
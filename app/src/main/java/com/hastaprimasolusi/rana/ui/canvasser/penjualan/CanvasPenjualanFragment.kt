 ...
 */
class CanvasPenjualanFragment: Fragment() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        viewModel.listProdKategori.clear()
        btnDetail.setOnClickListener {
            activity?.supportFragmentManager?.beginTransaction()
                ?.replace(R.id.frame, PosDetailFragment())
                ?.addToBackStack(null)
                ?.commit()
        }
        initViewModel()
    ...
    private fun initViewModel(){
        viewModel.listKategori.observe(viewLifecycleOwner, Observer {
            if(it.isNullOrEmpty()){
    ...
    private fun updateSummary(data: List<CartProdukModel>){
        ...
        data.forEach {
            it.pRODPRICE?.let { price ->
                ...
                jml += (price.first().pRODQTY.toString().toIntOrNull() ?: 0) * (price.first().pRODPRICE.toString().toIntOrNull() ?: 0)
            }
        }

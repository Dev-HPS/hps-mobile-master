package com.hastaprimasolusi.rana.ui.lp.akun
...
import kotlinx.android.synthetic.main.fragment_canvas_profile.txtSaldo
...
/**
 * Created By maasrahman on 8/25/20
 */
class LpProfileFragment: Fragment() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        lblPemilik.visibility = View.VISIBLE
        txtPemilik.visibility = View.VISIBLE
    ...
    private fun initViewModel(){
        ...
        viewModel.getProfile({
            val saldo = (it.uSERACCOUNTINFO?.aCCOUNTBALANCE?.toDouble()?.roundToInt() ?: 0).toString()
            txtAccountNo.text = "No. Akun ${it.uSERACCOUNTINFO?.aCCOUNTNUMBER}"
            txtSaldo.text = convertCurrency(saldo, 3, '.', UtilsPref.loadString(getString(R.string.currencySymbol)))
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 8 && resultCode == Activity.RESULT_OK) {
            val path = ImageFilePath.getPath(requireContext(), data?.data)
            GlobalScope.launch {
